...
class PromptVersionRegistry:
    ...
    def _build_rubric(
        self, recent_job_outcomes: list[dict[str, Any]], reviewer_artifacts: list[dict[str, Any]]
    ) -> dict[str, float]:
        ...
        avg_static_findings = sum(artifact.get("static_findings", 0) for artifact in reviewer_artifacts) / max(
            1, len(reviewer_artifacts)
        )
    ...
    def try_promote(
        ...
        *,
        ...
        safety_ok = all(
            safety_metrics.get(k, 0.0) >= baseline_safety_metrics.get(k, 0.0) for k in baseline_safety_metrics
        )
        ...
        if not gate_ok:
            event = {
                ...
                "rubric": rubric,
                "held_out_score": held_out_score,
                "eval_input": eval_input.__dict__,
