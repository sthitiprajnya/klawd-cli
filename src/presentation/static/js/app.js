...
function renderJobs(jobs) {
    ...
    jobsContainer.innerHTML = jobs.map((job) => `
        <div class="job-item" data-job-id="${escapeHTML(job.job_id)}">
            <div class="job-info">
                <span class="job-id" title="ID: ${escapeHTML(job.job_id)}">ID: ${escapeHTML(job.job_id).substring(0,12)}${job.job_id.length > 12 ? '...' : ''}</span>
                <span class="job-task" title="${escapeHTML(job.task)}">${escapeHTML(job.task).substring(0, 60)}${job.task.length > 60 ? '...' : ''}</span>
            </div>
