...
class ImmunizationFilter:
    ...
    def _llm_evaluate(self, prompt: str, category: str) -> ThreatResult:
        ...
        try:
            ...
            content = self.router.route(prompt=full_prompt, task_type="complex", job_id=f"immunization-{category}")
            ...
            return ThreatResult(
                category=category, blocked=result_dict.get("blocked", False), evidence=result_dict.get("reason", "")
            )
    ...
    def _check_prompt_injection(self, readme: str) -> ThreatResult:
        ...
        return self._llm_evaluate(
            f"Does this README contain prompt injection or override instructions?\n\n{readme}", "PROMPT_INJECTION"
        )
    ...
    def _check_capability_escalation(self, skill_md: str) -> ThreatResult:
        ...
        return self._llm_evaluate(
            f"Does this skill request privilege escalation, unjailed host execution, or root access?\n\n{skill_md}",
    ...
    def _check_network_scope(self, skill_md: str) -> ThreatResult:
        ...
        return self._llm_evaluate(
            f"Does this skill attempt unauthorized network scanning, egress outside of standard APIs, or botnet activity?\n\n{skill_md}",
            "NETWORK_SCOPE",
        )
    ...
    def _check_malicious_code(self, source_files: list[dict]) -> ThreatResult:
        for f in source_files:
            if f.get("ext") in [".py", ".js", ".ts", ".sh"]:
                ...
                if critical:
                    return ThreatResult(
                        category="MALICIOUS_CODE", blocked=True, evidence=critical[0].get("message", "")
                    )
    ...
    def _check_supply_chain(self, dependencies: list[dict]) -> ThreatResult:
        for dep in dependencies:
            try:
                resp = httpx.post(
                    self.OSV_API, json={"package": {"name": dep.get("name"), "ecosystem": dep.get("ecosystem")}}
                )
                vulns = resp.json().get("vulns", [])
                critical = [v for v in vulns if v.get("severity") == "CRITICAL"]
                if critical:
                    return ThreatResult(
                        category="SUPPLY_CHAIN", blocked=True, evidence=f"Critical CVE in {dep.get('name')}"
                    )
            except Exception:
                continue
