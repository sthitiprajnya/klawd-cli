...
def test_llm_router_initialization():
    from src.infrastructure.llm_router import LLMRouter
    test_router = LLMRouter()
    assert test_router.base_url == "http://litellm-proxy:4000/v1"

@patch('src.infrastructure.llm_router.OpenAI')
def test_llm_router_routes_models(mock_openai):
    ...
    # Fast task
    res_fast = test_router.route("Help", task_type="fast")
